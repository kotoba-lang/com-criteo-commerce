(ns criteo_commerce.main
  "Kotodama WASM entrypoint for the Criteo_commerce clean-room actor (L4) — Clojure port.
  ...
  facts (`criteo_commerce.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "criteo_commerce")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"criteo_commerce.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "criteo_commerce-compat" :tier tier :entities entities} 200])
...
(defn start! [] :criteo_commerce-compat/ready)
