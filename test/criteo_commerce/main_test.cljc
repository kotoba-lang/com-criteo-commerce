(ns criteo_commerce.main-test
  "Contract + behavioral test for the criteo_commerce-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "criteo_commerce-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'criteo_commerce.main-test)]
